...
// To enable pointer optimization for `opt::option<T *>`,
// define `OPT_OPTION_PTR_OPTIMIZATION` before including this header.
...
namespace opt {
    ...
    public:
        ...
        constexpr option(const std::optional<U> &std_opt) noexcept(std::is_nothrow_copy_constructible_v<U>)
            requires (!std::is_void_v<U> && !std::is_reference_v<U> && std::constructible_from<T, U>)
        {
    ...
    // needs to specify `<T &>`
    template <typename T>
    constexpr option<T> some(std::remove_reference_t<T> &value)
    ...
    constexpr option<T> some(Ts &&...args) noexcept(std::is_nothrow_constructible_v<option<T>, Ts &&...>) {
        return option<T>{ T(std::forward<Ts>(args)...) };
    }
