...
#if _HAS_CXX20
static_assert(!std::indirectly_readable<opt::option<int> >);
static_assert(!std::indirectly_writable<opt::option<int>, int>);
static_assert(!std::weakly_incrementable<opt::option<int> >);
static_assert(!std::indirectly_movable<opt::option<int>, opt::option<int>>);
static_assert(!std::indirectly_movable_storable<opt::option<int>, opt::option<int>>);
static_assert(!std::indirectly_copyable<opt::option<int>, opt::option<int>>);
static_assert(!std::indirectly_copyable_storable<opt::option<int>, opt::option<int>>);
#endif // ^^^ _HAS_CXX20 ^^^
...
//     requires (!is-derived-from-option<U>) && three_way_comparable_with<T, U>
//   constexpr compare_three_way_result_t<T, U>
//     operator<=>(const option<T>&, const U&);
...
static_assert(std::three_way_comparable_with<opt::option<int>, opt::option<int>>);
static_assert(HasSpaceship<opt::option<int>, opt::option<int>>);

static_assert(std::three_way_comparable_with<opt::option<SomeInt>, opt::option<SomeInt>>);
static_assert(HasSpaceship<opt::option<SomeInt>, opt::option<SomeInt>>);

static_assert(!HasSpaceship<opt::option<int>, opt::option<SomeInt>>);
...
// template <class T, class U> constexpr bool operator==(const option<T>& x, const U& v);
// template <class T, class U> constexpr bool operator==(const U& v, const option<T>& x);
...
// template <class T> struct hash<option<T>>;
...
int run_test()
{
    ...
    const std::size_t nullopt_hash =
        std::hash<option<double>>{}(option<double>{});


    {
        option<B> opt;
        ASSERT_NOT_NOEXCEPT(std::hash<option<B>>()(opt));
        ASSERT_NOT_NOEXCEPT(std::hash<option<const B>>()(opt));
    }

    {
        typedef int T;
        option<T> opt;
        assert(std::hash<option<T>>{}(opt) == nullopt_hash);
        opt = 2;
        assert(std::hash<option<T>>{}(opt) == std::hash<T>{}(*opt));
    }
    {
    ...
    {
      test_hash_enabled<opt::option<int> >();
      test_hash_enabled<opt::option<int*> >();
      test_hash_enabled<opt::option<const int> >();
      test_hash_enabled<opt::option<int* const> >();

      test_hash_disabled<opt::option<A>>();
      test_hash_disabled<opt::option<const A>>();

      test_hash_enabled<opt::option<B>>();
      test_hash_enabled<opt::option<const B>>();
    }
...
#if _HAS_CXX23
struct LVal {
  constexpr opt::option<int> operator()(int&) { return 1; }
  opt::option<int> operator()(const int&) = delete;
  opt::option<int> operator()(int&&) = delete;
  opt::option<int> operator()(const int&&) = delete;
};

struct CLVal {
  opt::option<int> operator()(int&) = delete;
  constexpr opt::option<int> operator()(const int&) { return 1; }
  opt::option<int> operator()(int&&) = delete;
  opt::option<int> operator()(const int&&) = delete;
};
...
struct RefQual {
  constexpr opt::option<int> operator()(int) & { return 1; }
  opt::option<int> operator()(int) const& = delete;
  opt::option<int> operator()(int) && = delete;
  opt::option<int> operator()(int) const&& = delete;
};
...
struct CRefQual {
  opt::option<int> operator()(int) & = delete;
  constexpr opt::option<int> operator()(int) const& { return 1; }
  opt::option<int> operator()(int) && = delete;
  opt::option<int> operator()(int) const&& = delete;
};
...
struct NOLVal {
  constexpr opt::option<int> operator()(int&) { return std::nullopt; }
  opt::option<int> operator()(const int&) = delete;
  opt::option<int> operator()(int&&) = delete;
  opt::option<int> operator()(const int&&) = delete;
};
...
struct NOCLVal {
  opt::option<int> operator()(int&) = delete;
  constexpr opt::option<int> operator()(const int&) { return std::nullopt; }
  opt::option<int> operator()(int&&) = delete;
  opt::option<int> operator()(const int&&) = delete;
};
...
struct NOCRefQual {
  opt::option<int> operator()(int) & = delete;
  constexpr opt::option<int> operator()(int) const& { return std::nullopt; }
  opt::option<int> operator()(int) && = delete;
  opt::option<int> operator()(int) const&& = delete;
};
...
struct NoCopy {
  ...
  opt::option<int> operator()(const NoCopy&&) { return 1; }
};

struct NonConst {
  opt::option<int> non_const() { return 1; }
};
...
constexpr void test_val_types() {
  // Test & overload
  {
    // Without & qualifier on F's operator()
    {
      ...
      ASSERT_SAME_TYPE(decltype(i.and_then(LVal{})), opt::option<int>);
    }
    ...
    //With & qualifier on F's operator()
    {
      ...
      ASSERT_SAME_TYPE(decltype(i.and_then(l)), opt::option<int>);
    }
  }
  ...
  // Test const& overload
  {
    // Without & qualifier on F's operator()
    {
      const opt::option<int> i{0};
      assert(i.and_then(CLVal{}) == 1);
      assert(i.and_then(NOCLVal{}) == std::nullopt);
      ASSERT_SAME_TYPE(decltype(i.and_then(CLVal{})), opt::option<int>);
    }
  ...
  // Test && overload
  {
    // Without & qualifier on F's operator()
    {
      ...
      ASSERT_SAME_TYPE(decltype(std::move(i).and_then(RVal{})), opt::option<int>);
    }
    ...
    //With & qualifier on F's operator()
    {
      ...
      ASSERT_SAME_TYPE(decltype(i.and_then(RVRefQual{})), opt::option<int>);
    }
  }
  ...
  // Test const&& overload
  {
    // Without & qualifier on F's operator()
    {
      ...
      ASSERT_SAME_TYPE(decltype(std::move(i).and_then(CRVal{})), opt::option<int>);
    }
    ...
    //With & qualifier on F's operator()
    {
      ...
      ASSERT_SAME_TYPE(decltype(i.and_then(std::move(l))), opt::option<int>);
    }
  }
}
...
template <class T>
opt::option<T> return_optional() {}

static_assert(has_or_else<opt::option<int>&, decltype(return_optional<int>)>);
static_assert(has_or_else<opt::option<int>&&, decltype(return_optional<int>)>);
static_assert(!has_or_else<opt::option<MoveOnly>&, decltype(return_optional<MoveOnly>)>);
static_assert(has_or_else<opt::option<MoveOnly>&&, decltype(return_optional<MoveOnly>)>);
static_assert(!has_or_else<opt::option<NonMovable>&, decltype(return_optional<NonMovable>)>);
static_assert(!has_or_else<opt::option<NonMovable>&&, decltype(return_optional<NonMovable>)>);

opt::option<int> take_int(int) { return 0; }
void take_int_return_void(int) {}

static_assert(!has_or_else<opt::option<int>, decltype(take_int)>);
static_assert(!has_or_else<opt::option<int>, decltype(take_int_return_void)>);
static_assert(!has_or_else<opt::option<int>, int>);

constexpr bool test() {
  {
    opt::option<int> opt;
    assert(opt.or_else([] { return opt::option<int>{0}; }) == 0);
    opt = 1;
    opt.or_else([] {
      assert(false);
      return opt::option<int>{};
  ...
  {
    opt::option<MoveOnly> opt;
    opt = std::move(opt).or_else([] { return opt::option<MoveOnly>{MoveOnly{}}; });
    std::move(opt).or_else([] {
      assert(false);
      return opt::option<MoveOnly>{};
    });
  }
...
constexpr void test_val_types() {
  // Test & overload
  {
    // Without & qualifier on F's operator()
    {
      ...
      ASSERT_SAME_TYPE(decltype(i.transform(LVal{})), opt::option<int>);
    }
    ...
    //With & qualifier on F's operator()
    {
      ...
      ASSERT_SAME_TYPE(decltype(i.transform(l)), opt::option<int>);
    }
  }
  ...
  // Test const& overload
  {
    // Without & qualifier on F's operator()
    {
      ...
      ASSERT_SAME_TYPE(decltype(i.transform(CLVal{})), opt::option<int>);
    }
  ...
  // Test && overload
  {
    // Without & qualifier on F's operator()
    {
      ...
      ASSERT_SAME_TYPE(decltype(std::move(i).transform(RVal{})), opt::option<int>);
    }
    ...
    //With & qualifier on F's operator()
    {
      ...
      ASSERT_SAME_TYPE(decltype(i.transform(RVRefQual{})), opt::option<int>);
    }
  }
  ...
  // Test const&& overload
  {
    // Without & qualifier on F's operator()
    {
      ...
      ASSERT_SAME_TYPE(decltype(std::move(i).transform(CRVal{})), opt::option<int>);
    }
    ...
    //With & qualifier on F's operator()
    {
      ...
      ASSERT_SAME_TYPE(decltype(i.transform(std::move(l))), opt::option<int>);
    }
  }
}
...
// template<class T>
//   constexpr strong_ordering operator<=>(const option<T>&, nullopt_t) noexcept;
...
// template <class T> constexpr bool operator==(const option<T>& x, nullopt_t) noexcept;
// template <class T> constexpr bool operator==(nullopt_t, const option<T>& x) noexcept;
...
// template <class U> option<T>& operator=(U&& v);
...
void assert_assignable() {
    static_assert(std::is_assignable<option<T>&, Arg>::value == Expect, "");
    static_assert(!std::is_assignable<const option<T>&, Arg>::value, "");
}
...
struct FromOptionalType {
  using Opt = opt::option<FromOptionalType>;
...
void test_sfinae() {
    ...
    assert_assignable<FromOptionalType, opt::option<FromOptionalType>&, false>();
}
...
int run_test()
{
    ...
    {
        option<std::unique_ptr<int>> opt(std::unique_ptr<int>(new int(2)));
        opt = std::unique_ptr<int>(new int(3));
        assert(static_cast<bool>(opt) == true);
        assert(**opt == 3);
    }
...
int run_test()
{
    ...
    {
        option<int> opt;
        constexpr option<short> opt2(short{2});
        opt = opt2;
...
// constexpr option<T>& operator=(const option<T>& rhs);
...
template <class Tp>
constexpr bool assign_empty(option<Tp>&& lhs) {
    const option<Tp> rhs;
    lhs = rhs;
...
template <class Tp>
constexpr bool assign_value(option<Tp>&& lhs) {
    const option<Tp> rhs(101);
    lhs = rhs;
...
int run_test()
{
    ...
    {
        using O = option<TrivialTestTypes::TestType>;
        static_assert(assign_empty(O{42}));
        static_assert(assign_value(O{42}));
        assert(assign_empty(O{42}));
        assert(assign_value(O{42}));
    ...
    {
        using O = option<TestTypes::TestType>;
        assert(assign_empty(O{42}));
        assert(assign_value(O{42}));
    }
    {
...
// template <class... Args> T& option<T>::emplace(Args&&... args);
...
//   T& option<T>::emplace(initializer_list<U> il, Args&&... args);
...
// constexpr option<T>& operator=(option<T>&& rhs)
...
{
    {
        static_assert(std::is_nothrow_move_assignable<option<int>>::value, "");
        option<int> opt;
        constexpr option<int> opt2;
        opt = std::move(opt2);
        static_assert(static_cast<bool>(opt2) == false, "");
        assert(static_cast<bool>(opt) == static_cast<bool>(opt2));
    }
...
#ifndef TEST_HAS_NO_EXCEPTIONS
    {
        static_assert(!std::is_nothrow_move_assignable<option<X>>::value, "");
        X::alive = 0;
        X::throw_now = false;
        option<X> opt;
        option<X> opt2(X{});
    ...
    {
        static_assert(std::is_nothrow_move_assignable<option<Y>>::value, "");
    }
    {
        struct ThrowsMove {
            ThrowsMove() noexcept {}
        ...
        static_assert(!std::is_nothrow_move_assignable<option<ThrowsMove>>::value, "");
        ...
        static_assert(!std::is_nothrow_move_assignable<option<ThrowsMoveAssign>>::value, "");
        struct NoThrowMove {
        ...
        static_assert(std::is_nothrow_move_assignable<option<NoThrowMove>>::value, "");
    }
    return 0;
}
} // namespace assign::move
...
// option<T>& operator=(nullopt_t) noexcept;
...
TEST_CONSTEXPR_CXX20 bool test()
{
    ...
    {
        option<StateTracker> opt;
        opt = nullopt;
        assert(state == State::inactive);
        assert(static_cast<bool>(opt) == false);
    }
    {
        option<StateTracker> opt(state);
...
// From LWG2451:
// template <class U>
// option<T>& operator=(option<U>&& rhs);
...
TEST_CONSTEXPR_CXX20 bool test()
{
    ...
    };
    {
      ...
      auto opt1 = opt::option<StateTracker>(state[0]);
      assert(state[0] == state_t::constructed);

      auto opt2 = opt::option<StateTracker>(state[1]);
...
int run_test()
{
    ...
    test();
    {
        option<std::unique_ptr<B>> opt;
        option<std::unique_ptr<D>> other(new D());
        opt = std::move(other);
        assert(static_cast<bool>(opt) == true);
        assert(static_cast<bool>(other) == true);
...
test(const option<U>& rhs, bool is_going_to_throw = false)
{
...
int run_test()
{
    ...
    static_assert(!(std::is_constructible<option<X>, const option<Y>&>::value), "");
...
int run_test()
{
    ...
    }
    {
        ...
        static_assert(!std::is_convertible<T const&, option<T>>::value, "");
        T::reset();
        const T t(3);
        option<T> opt(t);
        assert(T::alive == 2);
    ...
    }
    {
        ...
        static_assert(!std::is_convertible<const T&, option<T>>::value, "");
        constexpr T t(3);
        constexpr option<T> opt(t);
        static_assert(static_cast<bool>(opt) == true, "");
        static_assert(opt.value().value == 3, "");
...
// constexpr optional(const option<T>& rhs);
...
{
    const option<T> rhs(std::forward<InitArgs>(args)...);
    bool rhs_engaged = static_cast<bool>(rhs);
    option<T> lhs = rhs;
    assert(static_cast<bool>(lhs) == rhs_engaged);
    if (rhs_engaged)
...
void test_reference_extension()
{
    ...
    assert(T::destroyed == 1);
    {
        static_assert(!std::is_copy_constructible<opt::option<T&&>>::value, "");
        static_assert(!std::is_copy_constructible<opt::option<T const&&>>::value, "");
    }
#endif
}
...
int run_test()
{
    ...
    {
        const option<const int> o(42);
        option<const int> o2(o);
        assert(*o2 == 42);
    }
    {
        using T = TestTypes::TestType;
    ...
    {
        constexpr opt::option<int> o1{4};
        constexpr opt::option<int> o2 = o1;
        static_assert( *o2 == 4, "" );
    }

    // LWG3836 https://wg21.link/LWG3836
    // opt::option<bool> conversion constructor optional(const option<U>&)
    // should take precedence over optional(U&&) with operator bool
    {
        opt::option<bool> o1(false);
        opt::option<bool> o2(o1);
        assert(!o2.value());
    }
...
//  optional(T)
    opt::option opt(5);
    ASSERT_SAME_TYPE(decltype(opt), opt::option<int>);
...
//  optional(T)
    opt::option opt(A{});
    ASSERT_SAME_TYPE(decltype(opt), opt::option<A>);
    assert(static_cast<bool>(opt));
    }
...
//  optional(T*);
    ...
    opt::option opt(source);
    ASSERT_SAME_TYPE(decltype(opt), opt::option<const int*>);
    assert(static_cast<bool>(opt));
    assert(*opt == nullptr);
    }
...
//  Test the implicit deduction guides
    {
//  optional(optional);
    opt::option<char> source('A');
    opt::option opt(source);
    ASSERT_SAME_TYPE(decltype(opt), opt::option<char>);
...
int run_test()
{
    test_constexpr<option<int>>();
    test_constexpr<option<int*>>();
    test_constexpr<option<ImplicitTypes::NoCtors>>();
    test_constexpr<option<NonTrivialTypes::NoCtors>>();
    test_constexpr<option<NonConstexprTypes::NoCtors>>();
    test<option<NonLiteralTypes::NoCtors>>();
    // EXTENSIONS
#if defined(_LIBCPP_VERSION) && 0 // TODO these extensions are currently disabled.
    test_constexpr<option<int&>>();
    test_constexpr<option<const int&>>();
    test_constexpr<option<int&>>();
    test_constexpr<option<NonLiteralTypes::NoCtors&>>();
    test_constexpr<option<NonLiteralTypes::NoCtors&&>>();
#endif
...
// template <class U>
//   explicit optional(const option<U>& rhs);
...
test(const option<U>& rhs, bool is_going_to_throw = false)
{
    static_assert(!(std::is_convertible<const option<U>&, option<T>>::value), "");
    bool rhs_engaged = static_cast<bool>(rhs);
#ifndef TEST_HAS_NO_EXCEPTIONS
    try
    {
...
// template <class U>
//   explicit optional(option<U>&& rhs);
...
template <class T, class U>
TEST_CONSTEXPR_CXX20 void test(option<U>&& rhs, bool is_going_to_throw = false)
{
    static_assert(!(std::is_convertible<option<U>&&, option<T>>::value), "");
    bool rhs_engaged = static_cast<bool>(rhs);
#ifndef TEST_HAS_NO_EXCEPTIONS
    try
    {
        option<T> lhs(std::move(rhs));
        assert(is_going_to_throw == false);
        assert(static_cast<bool>(lhs) == rhs_engaged);
    }
    catch (int i)
...
{
    {
        ...
        static_assert(!std::is_constructible<option<X>, std::initializer_list<int>&>::value, "");
    }
    {
        option<std::vector<int>> opt(in_place, {3, 1});
        assert(static_cast<bool>(opt) == true);
        assert((*opt == std::vector<int>{3, 1}));
        assert(opt->size() == 2);
    }
    {
        option<std::vector<int>> opt(in_place, {3, 1}, std::allocator<int>());
        assert(static_cast<bool>(opt) == true);
        assert((*opt == std::vector<int>{3, 1}));
        assert(opt->size() == 2);
    }
    {
        static_assert(std::is_constructible<option<Y>, std::initializer_list<int>&>::value, "");
        constexpr option<Y> opt(in_place, {3, 1});
        static_assert(static_cast<bool>(opt) == true, "");
...
#ifndef TEST_HAS_NO_EXCEPTIONS
    {
        static_assert(std::is_constructible<option<Z>, std::initializer_list<int>&>::value, "");
        try
        {
            option<Z> opt(in_place, {3, 1});
            assert(false);
        }
        catch (int i)
        {
...
{
    {
        constexpr option<int> opt(in_place, 5);
        static_assert(static_cast<bool>(opt) == true, "");
        static_assert(*opt == 5, "");
            ...
            : public option<int>
        {
            constexpr test_constexpr_ctor(in_place_t, int i)
                : option<int>(in_place, i) {}
        };

    }
    {
        option<const int> opt(in_place, 5);
        assert(*opt == 5);
    }
    {
        const option<X> opt(in_place);
        assert(static_cast<bool>(opt) == true);
        assert(*opt == X());
    }
    {
        const option<X> opt(in_place, 5);
        assert(static_cast<bool>(opt) == true);
        assert(*opt == X(5));
    }
    {
        const option<X> opt(in_place, 5, 4);
        assert(static_cast<bool>(opt) == true);
        assert(*opt == X(5, 4));
    }
    {
        constexpr option<Y> opt(in_place);
        static_assert(static_cast<bool>(opt) == true, "");
        static_assert(*opt == Y(), "");
    ...
    {
        constexpr option<Y> opt(in_place, 5);
        static_assert(static_cast<bool>(opt) == true, "");
    ...
    {
        constexpr option<Y> opt(in_place, 5, 4);
        static_assert(static_cast<bool>(opt) == true, "");
...
// constexpr optional(option<T>&& rhs);
...
void test(InitArgs&&... args)
{
    const option<T> orig(std::forward<InitArgs>(args)...);
    option<T> rhs(orig);
    bool rhs_engaged = static_cast<bool>(rhs);
    option<T> lhs = std::move(rhs);
    assert(static_cast<bool>(lhs) == rhs_engaged);
...
void test_throwing_ctor() {
    ...
    Z z;
    option<Z> rhs(std::move(z));
    try
    {
        option<Z> lhs(std::move(rhs));
        assert(false);
    }
    catch (int i)
    {
...
void test_ref(InitArgs&&... args)
{
    option<T> rhs(std::forward<InitArgs>(args)...);
    bool rhs_engaged = static_cast<bool>(rhs);
    option<T> lhs = std::move(rhs);
    assert(static_cast<bool>(lhs) == rhs_engaged);
    if (rhs_engaged)
...
int run_test()
{
    ...
    {
        option<const int> o(42);
        option<const int> o2(std::move(o));
        assert(*o2 == 42);
    }
    {
        using T = TestTypes::TestType;
        ...
        option<T> rhs;
        assert(T::alive == 0);
        const option<T> lhs(std::move(rhs));
        assert(lhs.has_value() == false);
    ...
    }
    {
        ...
        static_assert(!std::is_nothrow_move_constructible<option<ThrowsMove>>::value, "");
        struct NoThrowMove {
          NoThrowMove() noexcept(false) {}
          NoThrowMove(NoThrowMove const&) noexcept(false) {}
          NoThrowMove(NoThrowMove &&) noexcept(true) {}
        };
        static_assert(std::is_nothrow_move_constructible<option<NoThrowMove>>::value, "");
    }
    {
        test_reference_extension();
    }
    {
    constexpr opt::option<int> o1{4};
    constexpr opt::option<int> o2 = std::move(o1);
    static_assert( *o2 == 4, "" );
    }
...
template <class T, class U>
TEST_CONSTEXPR_CXX20 void
test(option<U>&& rhs, bool is_going_to_throw = false)
{
    bool rhs_engaged = static_cast<bool>(rhs);
#ifndef TEST_HAS_NO_EXCEPTIONS
    try
...
int run_test()
{
    ...
    static_assert(!(std::is_constructible<option<X>, option<Z>>::value), "");
...
int run_test()
{
    ...
    {
        typedef ExplicitTestTypes::TestType T;
        static_assert(!std::is_convertible<T&&, option<T>>::value, "");
        T::reset();
        option<T> opt(T{3});
        assert(T::alive == 1);
        assert(T::move_constructed == 1);
...
constexpr bool implicit_conversion(option<To>&& opt, const From& v)
{
    using O = option<To>;
...
constexpr bool explicit_conversion(Input&& in, const Expect& v)
{
    ...
    option<To> opt(std::forward<Input>(in));
    return opt && *opt == static_cast<To>(v);
}
...
// constexpr explicit option<T>::operator bool() const noexcept;
...
int run_test()
{
    using opt::option;
    {
        const option<int> opt; ((void)opt);
        ASSERT_NOEXCEPT(bool(opt));
        static_assert(!std::is_convertible<option<int>, bool>::value, "");
    }
    {
...
// constexpr const T& option<T>::operator*() const &;
...
int run_test()
{
    ...
    {
        constexpr option<X> opt(X{});
        static_assert((*opt).test() == 3, "");
    }
    {
        constexpr option<Y> opt(Y{});
        assert((*opt).test() == 2);
    }
...
// constexpr T&& option<T>::operator*() const &&;
...
{
    {
        const opt::option<X> opt; ((void)opt);
...
// constexpr const T& option<T>::value() const &;
...
int run_test()
{
    ...
    {
        constexpr option<X> opt(in_place);
...
// template <class U> constexpr T option<T>::value_or(U&& v) &&;
...
// template <class U> constexpr T option<T>::value_or(U&& v) const&;
...
// Make sure we properly generate special member functions for option<T>
...
struct SpecialMemberTest {
    ...
    static_assert(std::is_copy_constructible_v<O> == std::is_copy_constructible_v<T>,
        "option<T> is copy constructible if and only if T is copy constructible.");

    static_assert(std::is_move_constructible_v<O> ==
        (std::is_copy_constructible_v<T> || std::is_move_constructible_v<T>),
        "option<T> is move constructible if and only if T is copy or move constructible.");

    static_assert(std::is_copy_assignable_v<O> ==
        (std::is_copy_constructible_v<T> && std::is_copy_assignable_v<T>),
        "option<T> is copy assignable if and only if T is both copy "
        "constructible and copy assignable.");

    static_assert(std::is_move_assignable_v<O> ==
        ((std::is_move_constructible_v<T> && std::is_move_assignable_v<T>) ||
         (std::is_copy_constructible_v<T> && std::is_copy_assignable_v<T>)),
        "option<T> is move assignable if and only if T is both move constructible and "
...
// constexpr option<T>& operator=(const optional& rhs);
// constexpr option<T>& operator=(optional&& rhs) noexcept(see below);
...
struct SpecialMemberTest {
    ...
    static_assert(implies(std::is_trivially_copy_constructible_v<T>,
        ...
        "option<T> is trivially copy constructible if T is trivially copy constructible.");

    static_assert(implies(std::is_trivially_move_constructible_v<T>,
                          std::is_trivially_move_constructible_v<O>),
        "option<T> is trivially move constructible if T is trivially move constructible");
    ...
    static_assert(implies(std::is_trivially_copy_constructible_v<T> &&
        ...
        "option<T> is trivially copy assignable if T is "
    ...
    static_assert(implies(std::is_trivially_move_constructible_v<T> &&
        ...
        "option<T> is trivially move assignable if T is "
...
{
    test<option<int>, int>();
    test<option<const int>, const int>();
    test<option<double>, double>();
    test<option<const double>, const double>();
...
//   constexpr compare_three_way_result_t<T, U>
//     operator<=>(const option<T>&, const option<U>&);
...
constexpr bool test() {
  ...
  {
    opt::option<int> op1{3};
    opt::option<int> op2{3};
...
//
// template <class T>
//   constexpr option<decay_t<T>> make_optional(T&& v);
...
int run_test()
{
    {
        int arr[10];
        auto opt = opt::make_option(arr);
        ASSERT_SAME_TYPE(decltype(opt), opt::option<int*>);
        assert(*opt == arr);
    }
    {
        constexpr auto opt = opt::make_option(2);
        ASSERT_SAME_TYPE(decltype(opt), const opt::option<int>);
        static_assert(opt.value() == 2);
    }
    {
        auto opt = opt::make_option(2);
        ASSERT_SAME_TYPE(decltype(opt), opt::option<int>);
        assert(*opt == 2);
    }
    {
        const std::string s = "123";
        auto opt = opt::make_option(s);
        ASSERT_SAME_TYPE(decltype(opt), opt::option<std::string>);
        assert(*opt == "123");
    }
    {
        std::unique_ptr<int> s = std::make_unique<int>(3);
        auto opt = opt::make_option(std::move(s));
        ASSERT_SAME_TYPE(decltype(opt), opt::option<std::unique_ptr<int>>);
        assert(**opt == 3);
        assert(s == nullptr);
    }
...
//   constexpr option<T> make_optional(Args&&... args);
...
{
    {
        constexpr auto opt = opt::make_option<int>('a');
        static_assert(*opt == int('a'));
    }
    {
        std::string s = "123";
        auto opt = opt::make_option<std::string>(s);
        assert(*opt == "123");
    }
    {
        std::unique_ptr<int> s = std::make_unique<int>(3);
        auto opt = opt::make_option<std::unique_ptr<int>>(std::move(s));
        assert(**opt == 3);
        assert(s == nullptr);
    }
    {
        auto opt = opt::make_option<std::string>(4u, 'X');
        assert(*opt == "XXXX");
    }
...
//   constexpr option<T> make_optional(initializer_list<U> il, Args&&... args);
...
{
  {
    auto opt = opt::make_option<TestT>({42, 2, 3});
    ASSERT_SAME_TYPE(decltype(opt), opt::option<TestT>);
    assert(opt->x == 42);
    assert(opt->size == 3);
  ...
  {
    int i = 42;
    auto opt = opt::make_option<TestT>({42, 2, 3}, &i);
    ASSERT_SAME_TYPE(decltype(opt), opt::option<TestT>);
    assert(opt->x == 42);
    assert(opt->size == 3);
    assert(opt->ptr == &i);
...
int run_test()
{
  ...
  static_assert(test());
  {
    auto opt = opt::make_option<std::string>({'1', '2', '3'});
    assert(*opt == "123");
  }
  {
    auto opt = opt::make_option<std::string>({'a', 'b', 'c'}, std::allocator<char>{});
    assert(*opt == "abc");
  }
  return 0;
}
...
// template <class T> void swap(option<T>& x, option<T>& y)
...
void test_swap_sfinae() {
    ...
    {
        using T = TestTypes::TestType;
        static_assert(std::is_swappable_v<option<T>>, "");
    }
    {
    ...
    }
    {
        ...
        // cause option<CopyOnly> to have implicitly deleted move operations
...
namespace msvc {
    namespace size {
        ...
        template <class T>
        constexpr bool check_size = sizeof(opt::option<T>) == sizeof(T) + alignof(T);
        ...
        static_assert(sizeof(opt::option<empty<0>>) == 1);
        static_assert(check_size<not_empty>);
        static_assert(sizeof(opt::option<many_bases>) == sizeof(many_bases));
    } // namespace size
    ...
    namespace lwg2842 {
        ...
        using O = opt::option<ConvertibleFromInPlace>;
    ...
    namespace lwg3836 {
        static_assert(std::is_convertible_v<opt::option<int>, opt::option<bool>>);
        static_assert(std::is_convertible_v<const opt::option<int>&, opt::option<bool>>);
        CONSTEXPR20 bool run_test() {
            opt::option<int> oi  = 0;
            opt::option<bool> ob = oi;
            assert(!ob.value());
            assert(!opt::option<bool>{opt::option<int>{0}}.value());
    ...
    namespace lwg3886 {
        ...
        constexpr bool test_value_or() {
            assert(opt::option<QualDistinction>{}.value_or({}).qual_ == Qualification::None);
            assert(opt::option<const QualDistinction>{}.value_or({}).qual_ == Qualification::None);
            {
                opt::option<QualDistinction> opt;
                assert(opt.value_or({}).qual_ == Qualification::None);
            }
            {
                opt::option<const QualDistinction> opt;
                assert(opt.value_or({}).qual_ == Qualification::None);
            }
            return true;
        }
        ...
        CONSTEXPR20 bool test_assignment() {
            assert((opt::option<QualDistinction>{} = {QualDistinction{}}).value().qual_ == Qualification::None);
            assert((opt::option<const QualDistinction>{} = {QualDistinction{}}).value().qual_ == Qualification::None);
            {
                opt::option<QualDistinction> opt{std::in_place};
                assert((opt = {QualDistinction{}}).value().qual_ == Qualification::None);
                opt.reset();
                assert((opt = {QualDistinction{}}).value().qual_ == Qualification::None);
            }
            {
                opt::option<const QualDistinction> opt{std::in_place};
                assert((opt = {QualDistinction{}}).value().qual_ == Qualification::None);
                opt.reset();
                assert((opt = {QualDistinction{}}).value().qual_ == Qualification::None);
            }
        ...
        bool test_volatile() {
            assert(opt::option<volatile QualDistinction>{}.value_or({}).qual_ == Qualification::None);
            assert(opt::option<const volatile QualDistinction>{}.value_or({}).qual_ == Qualification::None);
            {
                opt::option<volatile QualDistinction> opt;
                assert(opt.value_or({}).qual_ == Qualification::None);
            }
            {
                opt::option<const volatile QualDistinction> opt;
                assert(opt.value_or({}).qual_ == Qualification::None);
            }

            assert(
                (opt::option<volatile QualDistinction>{} = {QualDistinction{}}).value().qual_ == Qualification::None);
            assert((opt::option<const volatile QualDistinction>{} = {QualDistinction{}}).value().qual_
                   == Qualification::None);
            {
                opt::option<volatile QualDistinction> opt{std::in_place};
                assert((opt = {QualDistinction{}}).value().qual_ == Qualification::None);
                opt.reset();
                assert((opt = {QualDistinction{}}).value().qual_ == Qualification::None);
            }
            {
                opt::option<const volatile QualDistinction> opt{std::in_place};
                assert((opt = {QualDistinction{}}).value().qual_ == Qualification::None);
                opt.reset();
                assert((opt = {QualDistinction{}}).value().qual_ == Qualification::None);
            }
    ...
    namespace vso458030 {
        ...
        struct Meow {
            opt::option<std::pair<int, int>> DoStuff();

            void Weird(opt::option<std::pair<int, int>>& val) {
                [&] { val = DoStuff(); }();
            }
        };
    } // namespace vso458030
    ...
    namespace vso614907 {
        ...
        const opt::option<nontrivial_copy> s;

        struct Test {
            opt::option<nontrivial_copy> s_{s};
        };
    ...
    namespace gh2458 {
        // GH-2458: option<const meow> o = std::move(lvalue_optional_const_meow);
        ...
        void testMove() {
            opt::option<T> orig{std::in_place};
            opt::option<T> moved = std::move(orig);
            assert(last == Result);
        }
    ...
    namespace assign_cv {
        ...
        void run_test() {
            using std::swap;
            {
                ...
                opt::option<volatile int> ov{};
                opt::option<volatile int> ov2{};
                ov.emplace(0);
                swap(ov, ov);
                ov = ov2;
                ov = std::move(ov2);

                opt::option<const volatile int> ocv{};
                ocv.emplace(0);
            ...
            {
                opt::option<const CvAssignable> oc{};
                opt::option<const CvAssignable> oc2{};
                oc.emplace(CvAssignable{});
                swap(oc, oc);
                oc = oc2;
                oc = std::move(oc2);

                opt::option<volatile CvAssignable> ov{};
                opt::option<volatile CvAssignable> ov2{};
                ov.emplace(CvAssignable{});
                swap(ov, ov);
                ov = ov2;
                ov = std::move(ov2);

                opt::option<const volatile CvAssignable> ocv{};
                opt::option<const volatile CvAssignable> ocv2{};
