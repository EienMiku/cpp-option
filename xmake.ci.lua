...
-- startswith so e.g. `--toolchain=msvc[vs=2026]` still matches "msvc"
local toolchain = get_config("toolchain") or ""

if toolchain:startswith("clang") then
    -- Clang (libc++)
...
elseif toolchain:startswith("gcc") then
    -- GCC (libstdc++)
    set_languages("clatest", "cxxlatest")
...
elseif toolchain:startswith("msvc") then
    -- MSVC (Windows)
    add_requires("gtest", {system = false})
    add_requires("benchmark", {system = false})
